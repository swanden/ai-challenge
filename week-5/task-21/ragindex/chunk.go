...
// Chunk — единица индексации с метаданными (усиление задания):
...
func chunkFixed(docs []Doc, size, overlap int) []Chunk {
	if size <= 0 {
		size = 500
	}
...
func chunkStructural(docs []Doc) []Chunk {
	var out []Chunk
	for _, d := range docs {
		switch d.Ext {
		case ".go":
...
func chunkMarkdown(d Doc) []Chunk {
	var out []Chunk
	section := "(intro)"
	var buf []string
	ord := 0
...
func chunkParagraphs(d Doc) []Chunk {
	var out []Chunk
	ord := 0
