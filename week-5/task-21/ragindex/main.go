...
func main() {
	...
	if *dryRun {
		...
		mustSave(&Index{Strategy: "fixed-size", ChunkSize: *size, Overlap: *overlap, Chunks: fixed}, filepath.Join(*out, "index-fixed.json"))
		mustSave(&Index{Strategy: "structural", Chunks: structural}, filepath.Join(*out, "index-structural.json"))
		return
	}
	...
	idxFixed := &Index{Strategy: "fixed-size", ChunkSize: *size, Overlap: *overlap}
...
func mustSave(idx *Index, path string) {
