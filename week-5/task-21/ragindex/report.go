...
func runReport(ctx context.Context, o reportOpts) error {
	...
	emb := NewOllamaEmbedder(o.baseURL, o.model)
	if o.maxChars > 0 {
	...
	idxFixed := &Index{Strategy: "fixed-size", ChunkSize: o.size, Overlap: o.overlap}
	fmt.Println("fixed-size:")
	if err := idxFixed.embedAll(ctx, emb, fixed); err != nil {
		return err
	}
	idxStruct := &Index{Strategy: "structural"}
	...
	for _, idx := range []*Index{idxStruct, idxFixed} {
		fmt.Printf("\n[%s]\n", idx.Strategy)
...
func showContrast(fixed, structural []Chunk) {
...
func firstBroken(cs []Chunk) *Chunk {
	for i := range cs {
...
func firstWhole(cs []Chunk) *Chunk {
	for i := range cs {
...
func capChunks(cs []Chunk, n int) []Chunk {
	if n > 0 && len(cs) > n {
		return cs[:n]
	}
	return cs
...
func ollamaReachable(ctx context.Context, e Embedder) bool {
...
func saveDry(out string, size, overlap int, fixed, structural []Chunk) {
	_ = (&Index{Strategy: "fixed-size", ChunkSize: size, Overlap: overlap, Chunks: fixed}).Save(filepath.Join(out, "index-fixed.json"))
	_ = (&Index{Strategy: "structural", Chunks: structural}).Save(filepath.Join(out, "index-structural.json"))
}
