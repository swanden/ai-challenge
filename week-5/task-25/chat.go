...
func (s *ChatSession) Ask(ctx context.Context, userMsg string) (ChatTurn, error) {
	...
	if len(hits) == 0 || best < s.knowThr {
		...
		ans, err := s.a.chatAnswer(ctx, s.cfg.Model, userMsg, hits, s.History, s.State)
		if err != nil {
			return ChatTurn{}, err
		}
		turn.Answer = ans
	...
	if ns, err := s.a.updateTaskMemory(ctx, s.cfg.Model, s.History, s.State); err == nil {
		s.State = ns
	}
	return turn, nil
}
...
func (a *Agent) chatAnswer(ctx context.Context, model anthropic.Model, query string, hits []ragcore.Hit, history []ChatTurn, ts TaskMemory) (string, error) {
	...
	out := strings.TrimSpace(collectText(msg))
	if out == "" {
...
func (a *Agent) updateTaskMemory(ctx context.Context, model anthropic.Model, history []ChatTurn, prev TaskMemory) (TaskMemory, error) {
	...
	ns, ok := parseTaskMemory(collectText(msg))
	if !ok {
