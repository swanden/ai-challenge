...
func main() {
	...
	// День 17: поднимаем свой MCP-сервер (weatherserver) по stdio и отдаём
	// его инструменты агенту. Доступны всегда; вызовы проходят через инварианты.
	mcpCmd := []string{"go", "run", filepath.Join(sourceDir(), "weatherserver")}
	mcptools, mcpStop, err := ConnectMCP(ctx, mcpCmd)
	if err != nil {
		log.Printf("MCP недоступен (%v) — работаю без инструментов", err)
	} else {
		defer mcpStop()
		agent.SetMCP(mcptools)
		fmt.Printf("MCP подключён: %d инструмент(ов) — %v\n", mcptools.Len(), mcptools.Names())
	}
