...
type Reply struct {
	...
	RAGUsed       bool     // день 22: ответ построен с RAG-контекстом
	Sources       []string // день 22: источники подмешанных чанков (source · section)
}
...
type Agent struct {
	...
	gen       Completer // день 27: сменный бэкенд текстовой генерации (облако/локаль)
	projCtx   string    // день 31: живой контекст проекта (git через MCP), см. SetProjectContext
}
...
func NewAgent(client anthropic.Client, model anthropic.Model, mem *Memory) *Agent {
	return &Agent{client: client, model: model, maxTokens: 1024, mem: mem,
		...
		gen:  newCloudCompleter(client, model)} // по умолчанию — облако; -backend local меняет
}
...
// SetCompleter (день 27) переключает бэкенд генерации текстовых флоу (чат, память
// задачи, рой, enforce). MCP-цикл tool-use остаётся на a.client (см. completer.go).
func (a *Agent) SetCompleter(c Completer) { a.gen = c }

// SetProjectContext (день 31) кладёт в агента ЖИВОЙ контекст проекта — то, что
// получено детерминированным пре-фетчем git-тулов через MCP (ветка, HEAD, файлы).
//
// Почему это отдельное поле, а не ещё один «источник» в hits: у RAG-чанков и
// git-вывода РАЗНАЯ природа. Чанк — это документ, на который можно сослаться и из
// которого можно процитировать (и мы верифицируем цитату по тексту чанка). Вывод
// `git branch` — это состояние мира на момент запроса, у него нет chunk_id, и
// цитировать его как источник знания нельзя. Поэтому он идёт в промпт отдельным
// блоком и НЕ участвует в верификации цитат (verifyQuotes его не увидит).
// Пустая строка = контекста нет, поведение прежнее (дни 24–25 не ломаются).
func (a *Agent) SetProjectContext(s string) { a.projCtx = strings.TrimSpace(s) }
func (a *Agent) ProjectContext() string     { return a.projCtx }
...
// AskRAG (день 22): как AskOnce, но с RAG-контекстом, который уходит ОТДЕЛЬНЫМ
// system-блоком (не в текст вопроса) — поэтому инвариантная проверка стоп-слов
// работает только по самому вопросу и не ложно-срабатывает на ретривнутом коде.
func (a *Agent) AskRAG(ctx context.Context, query string, p Policy, ragContext string) (Reply, error) {
	return a.ask(ctx, query, p, false, ragContext)
}
...
func (a *Agent) ask(ctx context.Context, query string, p Policy, store bool, ragContext string) (Reply, error) {
	...
	// finalReply — сборка финального ответа (используется в двух местах: обычный
	// выход из цикла и деградация, когда tool_use-блоков не нашлось).
	finalReply := func(msg *anthropic.Message, text string) Reply {
		if store {
		...
		return Reply{
			...
			RAGUsed: strings.TrimSpace(ragContext) != "",
		}
	}
...
// nonEmpty — страховка от ВТОРОГО способа получить невалидный запрос: tool_result
// с пустым текстовым блоком API тоже не принимает. Инструмент может честно вернуть
// пустую строку (например, «изменений нет» вырожденное в ""), и это не должно ронять
// весь диалог.
func nonEmpty(s string) string {
	if strings.TrimSpace(s) == "" {
		return "(инструмент вернул пустой ответ)"
	}
	return s
}
...
func (a *Agent) runTool(ctx context.Context, name string, input json.RawMessage) (string, bool) {
	...
	if a.inv != nil {
		...
		if a.llmCheck {
			if v, err := EnforceLLM(ctx, a.gen, a.inv, intent); err == nil && !v.Allowed {
