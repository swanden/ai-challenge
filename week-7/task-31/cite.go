...
func (a *Agent) groundedAnswer(ctx context.Context, model anthropic.Model, query string, hits []ragcore.Hit) (GroundedReply, error) {
	...
	// День 31: живой контекст проекта (git через MCP), если он задан.
	// Ключевое: он НЕ является источником для цитирования — это состояние мира,
	// а не документ. Поэтому вносим его отдельным блоком и явно запрещаем модели
	// выдавать его за chunk (иначе она сочинит chunk_id, и verifyQuotes покажет
	// цитату без совпадения — мы это уже проходили на дне 24).
	if a.projCtx != "" {
