...
Это тридцать первый день и начало последней недели челленджа. Проект я не переписываю заново, а
продолжаю накопительный агент: каталог `week-7/task-31/` — это копия вчерашнего `task-30/` с
одной новой возможностью. Новая возможность — команда `/help`: ассистент, который понимает
проект и отвечает на вопросы о нём. Отвечает он, соединяя в одном ответе два источника:
документацию проекта (через RAG) и его живое состояние — текущую ветку git, список файлов,
историю коммитов (через MCP).
...
Эта неделя в курсе — заключительная, и лекция посвящена тому, как собрать всё наработанное
вместе. В ней разбирались субагенты (веерный запуск, цепочки, маршрутизация), системный
инструментарий (реестр инструментов и их исполнение), построение AI-пайплайнов, обработка ошибок
с участием человека и метрики качества — задержка, стоимость, надёжность. Общая установка автора
курса на неделю: брать реальные продукты, серьёзно относиться к архитектуре и рефакторингу и
выйти из недели не с игрушечным демо, а с чем-то цельным.

Отдельно лекция требует ставить защиту («гейт») на опасные операции — удаление файлов,
`git push`, правку конфигов, — примерно по той же схеме, по какой раньше делались инварианты. Я
решил этот вопрос иначе, и это было осознанное решение; подробно объясняю его в разделе «Честные
находки».

Из реплик автора курса в чате было ещё два важных уточнения. Во-первых (13.07), на этой неделе
задания конкретные и работать нужно именно с git — впрочем, GitLab тоже засчитывается.
Во-вторых (14.07), с этой недели снова разрешена облачная модель, что сняло вопрос о переносе
переранжирования на локальную модель: оно остаётся облачным.

К моменту работы над днём 31 уже вышло задание дня 32 — реактивный пайплайн, где pull request
запускает ревью по документации и коду, а результат возвращается комментарием. Это повлияло на
часть решений дня 31, о чём я пишу ниже.

Ещё несколько реплик из чата касались масштаба. Кто-то спрашивал про крон для переиндексации,
кто-то жаловался, что при 700 файлах индексация занимает 20–30 минут. У меня корпус небольшой
(46 файлов), поэтому крон не нужен — честно отметил это в FAQ. Также обсуждали, можно ли взять
для ассистента другой репозиторий, а не текущий. Я осознанно взял свой проект, и почему — в
следующем разделе.
...
У выбора есть прямое следствие: папки `docs/` у проекта ещё не было, а значит день 31 включает в
себя ещё и написание проектной документации. Именно она вместе с README и составляет корпус, по
которому ищет ассистент:
...
Кроме документации, в индекс идёт и сам код агента (файлы `.go`). Это сделано потому, что день 32
требует поиска по «документации и коду», так что корпус я собрал сразу под оба дня.

### 2. MCP git-сервер: свой и только на чтение

Git-сервер (`gitserver/`) я написал по образцу серверов предыдущих дней — погоды (день 17) и
заметок (день 20). Каркас подключения `ConnectMCP` уже был готов, так что добавился просто третий
сервер в списке.

Инструментов я сделал четыре, а не один минимально необходимый: `git_branch`, `git_files`,
`git_diff` и `git_log`. Дело не в жадности, а в завтрашнем дне: ревью кода дня 32 без `git_diff`
не существует, и если сегодня сделать «только `git_branch`», завтра сервер придётся переписывать.

Сервер работает только на чтение, и это гарантировано самим устройством: он умеет вызывать лишь
разрешённый список подкоманд (`rev-parse`, `status`, `ls-files`, `diff`, `log`), поэтому что-либо
записать он не может в принципе.
...
На практике это разделение выглядит так. Когда пользователь пишет `/help` с вопросом, ассистент
сразу сам получает ветку и список файлов прямым вызовом инструментов (это ничего не стоит в
токенах) и параллельно ищет ответ в документации, а затем отвечает с источниками и цитатами.
А когда пользователь пишет обычную строку без `/help`, запускается обычный цикл работы с
инструментами, где модель сама решает, вызвать ли `git_diff` или `git_log`.

### 4. Порог «не знаю» в `/help` означает не то, что в дне 24

В дне 24 механизм ответа устроен так: если лучший результат поиска слабее порога, ассистент сразу
говорит «не знаю». Это правильно, когда источник ровно один. Но у `/help` источника два, и старый
порог здесь мешал бы. Скажем, на вопрос «какая сейчас ветка?» в документации не найдётся ничего —
и это нормально, ветки нет ни в одном README, — но ответ-то уже лежит в пре-фетче из git, и
старый порог зря бы его отбросил.

Поэтому в `/help` порог значит не «не знаю», а «документация не помогла», и дальше есть три
случая. Если нет ни фрагментов из документации, ни контекста проекта — честное «не знаю». Если
документация пуста, но контекст проекта есть — отвечаем только по нему, без цитат. А если
документация что-то нашла — обычный ответ с цитатами, поверх которого добавлен контекст проекта.
При этом сами дни 24–25 я не трогал, у них остался свой механизм.

### 5. Контекст проекта — не источник для цитирования

Текущая ветка git — это состояние мира, а не документ. У него нет идентификатора фрагмента, и
цитировать его как знание нельзя. Поэтому контекст проекта уходит в запрос к модели отдельным
блоком и не участвует в проверке цитат. Если бы участвовал, модель могла бы сослаться на
несуществующий идентификатор, и проверка пометила бы такую цитату как выдуманную, — это я уже
проходил в дне 24.
