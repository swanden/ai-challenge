...
func main() {
	...
	reviewDry := flag.Bool("review-dry", false, "день 32: не постить коммент в PR, только напечатать")
	...
	support33Mode := flag.Bool("support33", false, "день 33: одна команда для видео — /support по FAQ продукта + контекст тикета из CRM")
	indexFAQ := flag.String("index-faq", filepath.Join(sourceDir(), "ragindex-faq", "index-structural.json"), "день 33: индекс FAQ продукта (support/faq)")
	supportTicket := flag.String("ticket", "", "день 33: ID тикета для контекста /support (напр. T-5001)")
	supportKnow := flag.Float64("support-know-threshold", 3.0, "день 33: порог «FAQ не помог» для /support")

	// День 34: ассистент для работы с файлами проекта.
	file34Mode := flag.Bool("file34", false, "день 34: демонстрация файлового ассистента (два сценария: changelog + invariant-check)")
	fileApply := flag.Bool("apply", false, "день 34: разрешить запись файлов (с гейтом подтверждения y/N). Без флага — только diff")
	fileGoal := flag.String("goal", "", "день 34: цель для файлового ассистента, напр. «обнови changelog» (без -file34 демо)")

	// День 35: ассистент разбора логов ошибок (реальная задача).
	triage35Mode := flag.Bool("triage35", false, "день 35: демонстрация разбора логов (база знакомых проблем + разбор моделью)")
	indexKB := flag.String("index-kb", filepath.Join(sourceDir(), "ragindex-kb", "index-structural.json"), "день 35: индекс базы знакомых проблем (triage/kb)")
	triageLogPath := flag.String("log", "", "день 35: путь к файлу лога для разбора (пусто = stdin, вместе с -triage-one)")
	triageOne := flag.Bool("triage-one", false, "день 35: разобрать один лог из -log или stdin")
	triageKnow := flag.Float64("triage-know-threshold", 3.0, "день 35: порог «нет в базе» для разбора логов")
	gitScope := flag.String("git-scope", "", "день 31: каталог проекта для git_files (пусто = автоопределение week-7/task-35 внутри репозитория)")
	flag.Parse()
	...
	servers := []ServerSpec{
		...
		{Name: "crm", Purpose: "поддержка: тикеты и пользователи продукта (get_ticket, get_user, list_tickets, search_tickets; только чтение)",
			Cmd: []string{"go", "run", filepath.Join(sourceDir(), "crmserver")}},
	}
	...
	// День 33: ассистент поддержки. RAG по FAQ продукта + контекст тикета из CRM (MCP).
	if *support33Mode {
		faqRetriever, err := NewRetriever(*indexFAQ, *ollamaURL, *embedModel, *ragK)
		if err != nil {
			log.Fatalf("support33: %s\n\nсобери индекс FAQ:\n  go run ./%s/ragindex -docs %s/support/faq -out %s/ragindex-faq -ext .md",
				err.Error(), scope, scope, scope)
		}
		supCfg := rcfg
		supCfg.Corpus = "ru" // FAQ на русском → rewrite расширяет русскими синонимами
		if err := runSupport33(ctx, agent, faqRetriever, mcptools, supCfg, *supportKnow); err != nil {
			log.Fatalf("support33: %s", err.Error())
		}
		return
	}
	// День 34: файловый ассистент. Демонстрация двух сценариев или одиночная цель.
	if *file34Mode {
		faqRet, _ := NewRetriever(*indexFAQ, *ollamaURL, *embedModel, *ragK)
		if err := runFile34(ctx, agent, faqRet, mcptools, scope, *fileApply); err != nil {
			log.Fatalf("file34: %s", err.Error())
		}
		return
	}
	if *fileGoal != "" {
		faqRet, _ := NewRetriever(*indexFAQ, *ollamaURL, *embedModel, *ragK)
		if err := runFile(ctx, agent, faqRet, mcptools, *fileGoal, scope, *fileApply); err != nil {
			log.Fatalf("file: %s", err.Error())
		}
		return
	}

	// День 35: ассистент разбора логов ошибок. RAG по базе знакомых проблем + модель.
	if *triage35Mode || *triageOne {
		kbRet, err := NewRetriever(*indexKB, *ollamaURL, *embedModel, *ragK)
		if err != nil {
			log.Fatalf("triage35: %s\n\nсобери индекс базы:\n  go run ./%s/ragindex -docs %s/triage/kb -out %s/ragindex-kb -ext .md",
				err.Error(), scope, scope, scope)
		}
		trCfg := rcfg
		trCfg.Corpus = "ru"
		if *triageOne {
			if err := runTriage(ctx, agent, kbRet, *triageLogPath, trCfg, *triageKnow); err != nil {
				log.Fatalf("triage: %s", err.Error())
			}
		} else {
			if err := runTriage35(ctx, agent, kbRet, trCfg, *triageKnow, scope); err != nil {
				log.Fatalf("triage35: %s", err.Error())
			}
		}
		return
	}

	// Одиночный вопрос поддержки с конкретным тикетом: -ticket T-XXXX (без -support33 демо).
	if *supportTicket != "" {
		faqRetriever, err := NewRetriever(*indexFAQ, *ollamaURL, *embedModel, *ragK)
		if err != nil {
			log.Fatalf("support: %s", err.Error())
		}
		supCfg := rcfg
		supCfg.Corpus = "ru"
		q := strings.TrimSpace(strings.Join(flag.Args(), " "))
		if q == "" {
			q = "В чём проблема пользователя и как её решить?"
		}
		if err := runSupport(ctx, agent, faqRetriever, mcptools, q, *supportTicket, supCfg, *supportKnow); err != nil {
			log.Fatalf("support: %s", err.Error())
		}
		return
	}
	...
	if *review32 || *reviewCI || *reviewDemo {
		...
		rvcfg := rcfg
		rvcfg.Corpus = "code"
		revConf := defaultReviewConfig(rvcfg)
		if *reviewCI {
			in, err := reviewInputFromEnv(*diffFile, *reviewDry, *reviewStrict)
			if err != nil {
				log.Fatalf("review-ci: %s", err.Error())
			}
			if err := runReviewCI(ctx, agent, revRetriever, revConf, in); err != nil {
				log.Fatalf("review-ci: %s", err.Error())
			}
		} else {
	...
	// День 33: /support — ленивый ретривер по FAQ продукта.
	var supportRetriever *Retriever
	supportCfg := rcfg
	supportCfg.Corpus = "ru"
	...
	for {
		...
		case line == "/help" || strings.HasPrefix(line, "/help "):
			...
			continue
		case line == "/support" || strings.HasPrefix(line, "/support "):
			// День 33. /support = FAQ продукта (RAG) + контекст тикета (MCP CRM).
			// Синтаксис: /support [T-XXXX] <вопрос>. Первый токен, похожий на ID
			// тикета (T-…), трактуется как тикет для пре-фетча.
			rest := strings.TrimSpace(strings.TrimPrefix(line, "/support"))
			if rest == "" {
				fmt.Println("использование: /support [T-XXXX] <вопрос о продукте>")
				fmt.Println("  например: /support T-5003 не могу сохранить заметку")
				continue
			}
			ticketID := ""
			fields := strings.Fields(rest)
			if len(fields) > 0 && strings.HasPrefix(strings.ToUpper(fields[0]), "T-") {
				ticketID = strings.ToUpper(fields[0])
				rest = strings.TrimSpace(strings.TrimPrefix(rest, fields[0]))
			}
			if supportRetriever == nil {
				r, err := NewRetriever(*indexFAQ, *ollamaURL, *embedModel, *ragK)
				if err != nil {
					fmt.Printf("индекс FAQ не подключён: %s\n", err.Error())
					fmt.Printf("собери его: go run ./%s/ragindex -docs %s/support/faq -out %s/ragindex-faq -ext .md\n", scope, scope, scope)
					continue
				}
				supportRetriever = r
				fmt.Printf("FAQ продукта: %s\n", supportRetriever.Info())
			}
			if err := runSupport(ctx, agent, supportRetriever, mcptools, rest, ticketID, supportCfg, *supportKnow); err != nil {
				fmt.Printf("/support: %s\n", err.Error())
			}
			continue
		case line == "/file" || strings.HasPrefix(line, "/file "):
			// День 34: файловый ассистент. Цель ставится словами; -apply включает
			// гейт записи (по умолчанию только diff). Синтаксис: /file <цель>.
			goal := strings.TrimSpace(strings.TrimPrefix(line, "/file"))
			if goal == "" {
				fmt.Println("использование: /file <цель>")
				fmt.Println("  например: /file обнови changelog   или   /file проверь инварианты")
				fmt.Println("  запись включается флагом -apply при старте (иначе только diff)")
				continue
			}
			if err := runFile(ctx, agent, supportRetriever, mcptools, goal, scope, *fileApply); err != nil {
				fmt.Printf("/file: %s\n", err.Error())
			}
			continue
