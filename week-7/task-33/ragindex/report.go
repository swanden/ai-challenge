...
func runReport(ctx context.Context, o reportOpts) error {
	banner(1, "КОРПУС")
	corpus, err := loadCorpus(o.docs, strings.Split(o.exts, ","), defaultSkipDirs(), skipFiles())
	if err != nil {
		return err
	}
	if len(corpus) == 0 {
...
// defaultSkipDirs — каталоги, которые НЕ являются документацией проекта.
//
// День 31: корпус RAG теперь = сам каталог task-31 (README + docs/ + код агента).
// Значит из него надо вычесть всё, что проектом не является:
//   - corpus-ru   — 12 транскриптов лекций ВШЭ (чужой корпус недели 5, не наш проект);
//   - ragindex-*  — каталоги с артефактами индексов (в т.ч. index-*.json и comparison.md,
//     их же отсекает isArtifact — но каталог целиком дешевле);
//   - memory/notes/reports — рантайм-состояние агента, не документация.
//
// defaultSkipDirs — каталоги, которые НЕ являются документацией проекта.
//
// День 31: корпус RAG теперь = сам каталог task-N (README + docs/ + код агента).
// Значит из него надо вычесть всё, что проектом не является:
//   - corpus-ru   — 12 транскриптов лекций ВШЭ (чужой корпус недели 5, не наш проект);
//   - ragindex-*  — каталоги с артефактами индексов (в т.ч. index-*.json и comparison.md,
//     их же отсекает isArtifact — но каталог целиком дешевле);
...
func defaultSkipDirs() []string {
	return []string{
		"memory", "notes", "reports", "node_modules",
		"ragindex", "ragindex-ru", "ragindex-docs",
		"corpus-ru",
	}
}

// skipFiles — файлы, которые лежат внутри корпуса, но документацией ПРОЕКТА не являются.
//
// День 32, закрытие долга дня 31 (находка «корпус отравляет сам себя»). devlog.md — это
// рабочий журнал: история прогонов и разбор багов. Для человека это очевидно не «как
// устроен проект», а «как его чинили». Для retrieval — неотличимо: тот же markdown, те же
// термины, те же имена файлов. На дне 31 журнал (в составе README) разросся до ~40% корпуса
// и вытеснял docs/ из top-4 — ассистент отвечал на «что ещё меняет флаг» цитатой из
// объявления флага вместо раздела о последствиях. Вынесли журнал в docs/devlog.md и не
// индексируем его: в корпусе остаётся только то, что ОПИСЫВАЕТ проект.
func skipFiles() map[string]bool {
