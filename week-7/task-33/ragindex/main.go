...
func main() {
	...
	// День 31, баг с реального прогона: каталог -out НЕ создавался. Симптом был
	// обманчивым — comparison.md не записался (всего лишь log.Printf, идём дальше),
	// затем честно посчитались ВСЕ эмбеддинги, и только на сохранении индекса
	// процесс падал. Работа сделана и выброшена.
	//
	// Лечим оба симптома разом: создаём каталог заранее и падаем СРАЗУ, если не
	// получилось. Проверка того, что мы сможем записать результат, обязана стоять
	// ДО дорогой работы, а не после неё.
	if err := os.MkdirAll(*out, 0o755); err != nil {
		log.Fatalf("каталог -out %q: %v", *out, err)
	}
	...
	corpus, err := loadCorpus(*docs, strings.Split(*exts, ","), defaultSkipDirs(), skipFiles())
	if err != nil {
	...
	truncFixed := emb.Truncated

	// День 31: счётчик усечений — КУМУЛЯТИВНЫЙ по эмбеддеру. Раньше он печатался
	// одним числом в конце, и было непонятно, чья это беда. На реальном прогоне это
	// дало ложную тревогу: «усечено входов: 85» после того, как мы поставили потолок
	// размера чанка. Оказалось — все 85 из fixed-size, в structural ноль.
	// Обнуляем между стратегиями и отчитываемся раздельно.
	emb.Truncated = 0
	...
	// Усечение на эмбеддинге — это ТИХАЯ потеря recall: текст лежит в чанке целиком,
	// но в вектор попадает только его начало. Печатаем по стратегиям, потому что
	// у них разная природа проблемы.
	fmt.Printf("\nУсечено на эмбеддинге (лимит %d символов):\n", emb.MaxChars)
	fmt.Printf("  structural: %d из %d чанков", truncStruct, len(idxStruct.Chunks))
	if truncStruct == 0 {
		fmt.Print("  ← это индекс, который использует агент; потолок чанка держит\n")
	} else {
		fmt.Print("  ← ВНИМАНИЕ: потолок чанка не держит, часть текста не попадёт в вектор\n")
	}
	fmt.Printf("  fixed-size: %d из %d чанков", truncFixed, len(idxFixed.Chunks))
	if truncFixed > 0 {
		fmt.Printf("  ← ожидаемо: -chunk-size задан в СЛОВАХ (%d ≈ %d символов > лимита).\n",
			*size, *size*6)
		fmt.Println("               fixed-size — только baseline для сравнения, агент его не читает.")
	} else {
		fmt.Println()
	}
