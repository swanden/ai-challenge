...
// MemoryStore — реализация Store без диска. Нужна отчёту: каждый сценарий
// гоняем на чистом агенте, не трогая history.json пользователя.
// Заодно показывает, зачем нужен интерфейс Store из дня 7.
...
func (m *MemoryStore) Load() ([]storedMessage, error) { return m.data, nil }

func (m *MemoryStore) Save(history []storedMessage) error {
	m.data = append([]storedMessage(nil), history...) // копия, не ссылка
	return nil
}
